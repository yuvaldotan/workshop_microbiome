...
class BaboonModel:
    ...
    def fit(self, lambda_):
        ...
        def objective(alpha, lambda_):
            ...
            D_var = self.transformed_data.expanding().var() # calculates the population varience

            cos = 0.5*(1+ np.cos((2*np.pi*self.delta_t[2:].values)/365))

            likelihood = 1
            for row in range(2,len(self.data)):
                var = np.diag(D_var.iloc[row-2])
                mean = alpha @ self.transformed_data[row-1] * cos[row-2]
                likelihood *= 1/((2*np.pi)**(len(var)/2)*np.linalg.det(var)**0.5) * np.exp(-0.5*(self.transformed_data.iloc[row]-mean)@np.linalg.inv(var)@(self.transformed_data.iloc[row]-mean))
        ...
        optimezed_score = minimize(lambda a: objective(a,lambda_), x0 = self.alpha_.flatten(), method="L-BFGS-B", bounds=[(-1,1)]*(61*61), tol = 1e-3)

        self.alpha_ = optimezed_score.x.reshape(61,-1)
