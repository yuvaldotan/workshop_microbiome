...
class BaboonModel:
    ...
    def fit(self, alpha, gamma):
        ...
        D_meanI = self.weighted_mean(gamma).values
    ...
    def predict(self, alpha, gamma, iterative = False):
        # print(len(self.data_I), len(self.metadata_I))
        if iterative:
            return self.iterative_predictor(alpha, gamma)
        return self.non_iterative_predictor(alpha, gamma)
    
    def non_iterative_predictor(self,alpha, gamma):
        mask = self.metadata_I[~np.isin(self.metadata_I.index,self.data_I.index)].index # sample ids to predict
        D_I = self.weighted_mean(gamma, mask).values
    ...
    def iterative_predictor(self, alpha,gamma):
        mask = self.metadata_I[~np.isin(self.metadata_I.index,self.data_I.index)].index # sample ids to predict
        data = self.data_I.copy()
        for sample in mask:
            D_I = self.weighted_mean_row(gamma, sample)
    ...
    def weighted_mean(self,gamma, indexes = None):
        res = pd.DataFrame(columns=self.data_I.columns) 
        if indexes is None:
            indexes = self.data_I.index[1:]
        for idx in indexes:
            res.loc[idx] = self.weighted_mean_row(gamma,idx)
        return res
    
    def weighted_mean_row(self,gamma, idx):

        row = np.zeros(61)

        loc = self.delta_t.index.get_loc(idx)
        ...
        time_reduction = np.power(1e-10 + relative_delta_t, -gamma)
    
        weight = cos * time_reduction
        if len (weight) > len(self.data_I):
            weight = weight[:len(self.data_I)]
            return np.average(self.data_I.values, axis=0, weights=weight)
        else:
            return np.average(self.data_I.iloc[:len(weight)].values, axis=0, weights=weight)
        return row
...
class superModel:
    ...
    def fit(self):
        ...
        best_score = objective(self.alpha_, self.gamma_)
        
        for g in [0.1 ** (i-1) for i in range(51)]:
            score = objective(self.alpha_, g)
            if score < best_score:
                best_score= score
                self.gamma_ = g
        ...
        optimezed_gamma = minimize(lambda gamma: objective(self.alpha_, gamma), x0 = self.gamma_, method="L-BFGS-B", bounds=[(0,10)])
